...
client.on('connect', (connection) => {
    ...
    let intervalObj: NodeJS.Timeout | null = setInterval(() => {
...
console.log('wss://irc-ws.chat.twitch.tv:443')
...
function parseMessage(message: string): {
    ...
    command: null | {
        ...
        isCapRequestEnabled?: boolean | undefined;
    },
    ...
    let parsedMessage: {
        ...
        parameters: null | string
    } | null = {  // Contains the component parts.
        tags: null,
        source: null,
        command: null,
        parameters: null
...
function parseTags(tags: string) {
    ...
    let dictParsedTags: {
        [key: string]: { [key: string]: string } | null | string[] | string | {
            [key: string]: {
    ...
    parsedTags.forEach(tag => {
        ...
        switch (parsedTag[0]) {  // Switch on tag name
            ...
            case 'badge-info':
                ...
                if (tagValue) {
                    let dict: { [key: string]: string } = {};  // Holds the list of badge objects.
...
function parseCommand(rawCommandComponent: string): { command: string; channel?: string; isCapRequestEnabled?: boolean } | null {
    let parsedCommand: { command: string; channel?: string; isCapRequestEnabled?: boolean } | null = null;
...
function parseParameters(rawParametersComponent: string, command: {
    command: string;
    channel?: string;
